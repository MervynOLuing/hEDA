...
kmedoidsSolution<- function(strata,
  ...
  for (k in (unique(strata$DOM1))) {
    ...
    aggr <- aggrStrata(strata=stratacorr,
                       ...
                       dominio=k)
    v <- SamplingStrata::bethel(aggr, errorscorr, minnumstrat = minnumstrat)
    sum(v)
    best[k] <- sum(v)
    if (is.na(maxclusters)) {
      #times <- round(nrow(stratacorr)*0.5,0)
      times <- round(nrow(stratacorr)*0.05,0)
     # times <- round(nrow(stratacorr)*0.5,0)
    }
    if (!is.na(maxclusters)) {
      times <- min(maxclusters,round(nrow(stratacorr)*1,0))
    }
    if (showPlot == TRUE) {
      plot(1,sum(v),xlim=c(1,times),ylim=c(0,1.5*sum(v)),type="p",
           ylab="Sample size",xlab="Number of clusters")
      tit <- paste("title('kmedoids clustering in domain ",k,"')",sep="")
      eval(parse(text=tit))
    }
    bestsolution <- NULL
    ...
    for (i in min:max) {
      stmt3 <- paste("],",i,",cluster.only = TRUE,metric = 'euclidean', stand = FALSE)",sep="")
  ...
  solutionKmedoid <- as.data.frame(cbind(suggestions, domainvalue),
