                           ...
                           showPlot = TRUE)
{
  ...
  for (k in (unique(strata$DOM1))) {
    ...
    aggr <- aggrStrata(strata = stratacorr, nvar = nvariables,
                       censiti = 0, vett = rep(1, nrow(stratacorr)), dominio = k)
    ...
    if (showPlot == TRUE) {
      plot(1, sum(v), xlim = c(1, times), ylim = c(0, 1.5 *
                                                     sum(v)), type = "p", ylab = "Sample size",
    ...
    for (i in min:max) {
      stmt3 <- paste("],", i, ",1000,algorithm='MacQueen')$cluster",
                     sep = "")
      stmt <- paste(stmt1, stmt2, stmt3, sep = "")
      eval(parse(text = stmt))
      aggr <- aggrStrata(strata = stratacorr, nvar = nvariables,
                         censiti = 0, vett = solution, dominio = k)
      v <- SamplingStrata::bethel(aggr, errorscorr, minnumstrat = minnumstrat)
      if (showPlot == TRUE)
        points(i, sum(v))
      if (sum(v) <= best[k]) {
